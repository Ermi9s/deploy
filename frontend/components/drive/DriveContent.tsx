...
interface DriveContentProps {
  ...
  /** Active search query — used to show a better empty state */
  searchQuery?: string
  ingestionByDocumentId: Record<string, IngestionStatusSnapshot>
  ...
  if (items.length === 0) {
    ...
    return (
      <div className="px-4 py-10 text-center">
        {trimmedQuery ? (
          <>
            <p className="font-medium text-foreground">No results for &ldquo;{trimmedQuery}&rdquo;</p>
...
const RANKING_LABELS: Record<number, { label: string; color: string }> = {
  1: { label: 'Public',       color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  2: { label: 'Restricted',   color: 'bg-sky-50 text-sky-700 border-sky-200' },
  3: { label: 'Confidential', color: 'bg-amber-50 text-amber-700 border-amber-200' },
  4: { label: 'Secret',       color: 'bg-orange-50 text-orange-700 border-orange-200' },
  5: { label: 'Top Secret',   color: 'bg-red-50 text-red-700 border-red-200' },
}
        ...
        <TabsContent value="metadata" className="mt-3 min-h-0 flex-1 overflow-y-auto">
          {selectedItem ? (
            <div className="space-y-3 text-sm">
              ...
              {selectedItem.type === 'file' && (() => {
                const accessEntries = Object.entries(selectedItem.departmentAccess ?? {})
                    ...
                    {isPrivate ? (
                      ...
                      <ul className="space-y-1.5">
                        {accessEntries.map(([deptId, minRanking]) => (
                          <li
                            key={deptId}
                            className="flex items-center justify-between gap-2 rounded-lg border border-border bg-muted/40 px-3 py-1.5"
                          >
                            <span className="text-xs font-semibold text-foreground/90 truncate max-w-[150px]" title={deptId}>
