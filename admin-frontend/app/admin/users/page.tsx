...
import { api, AuthUser, Department } from '@/lib/api'
import { Users, Search, Network, KeyRound, Loader2, ShieldCheck, X, Shield } from 'lucide-react'
...
export default function AdminUsers() {
  ...
  const loadData = async () => {
    try {
      ...
      toast({ title: 'Error', description: err instanceof Error ? err.message : 'Failed to fetch directory', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }

  useEffect(() => { loadData() }, [searchQuery, selectedDeptId, page])
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Success', description: `Profile assignments for ${selectedUser.email} updated successfully.` })
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800/80 bg-slate-900/50">
                      <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">User Identity</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">Department</th>
                      <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-400">Clearance Tier</th>
                  ...
                  <tbody className="divide-y divide-slate-800/40">
                    {loading && users.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-16"><Loader2 className="w-6 h-6 animate-spin text-indigo-500 mx-auto" /></td></tr>
                    ) : users.length > 0 ? (
                      users.map((u) => (
                        <tr key={u.id} className="hover:bg-slate-800/20 transition-all duration-150 cursor-pointer" onClick={() => handleOpenEdit(u)}>
                          <td className="px-6 py-4">
                            <p className="text-sm font-semibold text-slate-200">{u.first_name || u.last_name ? `${u.first_name} ${u.last_name}` : 'No Name Set'}</p>
                          ...
                          <td className="px-6 py-4 text-sm text-slate-300">
                            {u.profile?.permission_level?.name ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-800 text-slate-400 text-xs font-medium">Rank 1 - Public</span>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            {u.is_superuser ? (
                          ...
                          <td className="px-6 py-4 text-right">
                            <Button variant="ghost" size="sm" className="text-indigo-400 hover:text-indigo-300 hover:bg-slate-800 font-semibold"
                      ...
                      <tr><td colSpan={5} className="text-center py-16 text-slate-500 text-sm">No users matched your query filter.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          {selectedUser ? (
            <Card className="border-slate-800 bg-slate-900/30 backdrop-blur-md sticky top-24">
              <CardHeader className="border-b border-slate-800/60 pb-6 flex flex-row items-start justify-between">
                ...
                <button onClick={() => setSelectedUser(null)} className="text-slate-400 hover:text-slate-200"><X className="w-4 h-4" /></button>
              </CardHeader>
              <CardContent className="p-6">
                <form onSubmit={handleSaveUser} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <select value={editDeptId} onChange={(e) => { setEditDeptId(e.target.value); setEditLevelId('') }}
                      className="w-full px-4 py-3 bg-slate-950 border border-slate-800 text-slate-300 rounded-xl focus:border-indigo-500 text-sm focus:outline-none">
                  ...
                  <div className="space-y-2">
                    ...
                    <select value={editLevelId} onChange={(e) => setEditLevelId(e.target.value)} disabled={!editDeptId}
                      className="w-full px-4 py-3 bg-slate-950 border border-slate-800 text-slate-300 rounded-xl focus:border-indigo-500 text-sm focus:outline-none disabled:opacity-50">
                      <option value="">Select Level...</option>
                      {currentLevels.sort((a, b) => a.ranking - b.ranking).map((level) => (
                        <option key={level.id} value={level.id}>Rank {level.ranking} - {level.name}</option>
                      ))}
                    </select>
                  </div>
                  <div className="flex items-center justify-between p-4 rounded-xl border border-slate-800 bg-slate-950">
                    ...
                    <Switch checked={editIsSuperuser} onCheckedChange={setEditIsSuperuser}
                      disabled={selectedUser.is_superuser && selectedUser.email === 'admin@okm.local'} />
                  </div>
                  <div className="flex gap-3 pt-2">
                    <Button type="button" variant="outline" className="w-full border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-slate-200" onClick={() => setSelectedUser(null)}>Cancel</Button>
                    <Button type="submit" disabled={submitting} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white">{submitting ? 'Updating...' : 'Apply Access'}</Button>
                  </div>
                </form>
              </CardContent>
            </Card>
